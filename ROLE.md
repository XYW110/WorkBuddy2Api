...
对同时满足以下条件的任务，可跳过 Plan Agent，走快速路径：

1. 改动文件数不超过 2 个。
2. 不新增 store 字段 / API / 路由 / 依赖。
3. 用户已给出明确代码片段或改法（如 UI + handler）。
4. 仅接线或局部文案/样式调整，不涉及业务逻辑重构。

快速路径：搜索定位目标 -> 顺序编辑（禁止对同一文件并行多处改动）-> 读文件确认 -> 诊断/验证 -> 结果汇报。

仍须保留：TODO 管理、质量验证、结果汇报。执行中若发现超出预期复杂度，立即暂停并回退到完整「一步一规划」流程。

复杂任务（跨文件逻辑、新增链路、不确定现有结构）仍必须走完整规划流程。

### 核心原则

确保每一步都进行规划，以多次规划实现更高编码质量与更低返工率；简单任务用简化豁免换速度，复杂任务用完整规划控风险。
